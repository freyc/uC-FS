...
static xSemaphoreHandle fs_semaphore;
static xSemaphoreHandle   **FS_OS_DevLockSemTbl;
static xSemaphoreHandle   **FS_OS_DevAccessLockSemTbl;
...
void  FS_OS_Lock (FS_ERR  *p_err)
{
    ...
    xSemaphoreTake(fs_semaphore, portMAX_DELAY);
   *p_err = FS_ERR_NONE;
}
...
CPU_BOOLEAN  FS_OS_SemCreate (FS_OS_SEM  *p_sem,
                              CPU_INT16U  cnt)
{
   *p_sem = xSemaphoreCreateCounting(0xffff, cnt);
    return (DEF_OK);
}
                            ...
                            CPU_INT32U  timeout)
{
    ...
    if(xSemaphoreTake(*p_sem, pdMS_TO_TICKS(timeout) ) == pdTRUE) {
        return DEF_OK;
    }
